	...
	<div class="layout-padding" v-loading="loading">
		<div class="layout-padding-auto layout-padding-view">
				...
				<div class="option-bar" style="width: 100%">
					<div>
						...
						<!-- <el-button
							...
							:disabled="selectedId == '' || plan.state == 1"
							class="add-btn"
							v-auth="'progress_dpmTaskPlan_export'"
							@click="test"
						>
							test
						</el-button> -->
			...
			<el-table
				...
				:load="loadData"
				:cell-class-name="cellBell"
				:tree-props="{ children: 'children', hasChildren: 'hasChild' }"
			>
				<el-table-column type="selection" width="40px" align="center" />
				<el-table-column prop="nodeStr" label="标识号" width="280px" show-overflow-tooltip align="left" />
				<el-table-column prop="taskName" label="任务名称" min-width="400px" show-overflow-tooltip align="left">
					<template #default="{ row, $index }">
						<el-form :ref="`formRef${$index}ts`" v-show="row.isEdit" :model="row" class="flag-info-id">
							<el-form-item prop="taskName" :required="true" :rules="dataRules.taskName">
								<el-input v-model="row['taskName']" placeholder="请输入" autofocus :maxlength="255" />
				...
				<el-table-column prop="planBeginDate" label="计划开始时间" width="120" show-overflow-tooltip align="left">
					<template #default="{ row, $index }">
						<el-form :ref="`formRef${$index}ts`" v-show="row.isEdit" :model="row">
							<el-form-item prop="planBeginDate" :required="true" :rules="dataRules.planBeginDate">
								<el-date-picker
				...
				<el-table-column prop="planEndDate" label="计划完成时间" width="120" show-overflow-tooltip align="left">
					<template #default="{ row, $index }">
						<el-form :ref="`formRef${$index}ts`" v-show="row.isEdit" :model="row">
							<el-form-item prop="planEndDate" :required="true" :rules="dataRules.planEndDate">
								<el-date-picker
				...
				<el-table-column prop="associatedBusiness" label="关联业务" width="165px" show-overflow-tooltip align="left">
					<template #default="{ row, $index }">
						<div>{{ row.associatedBusiness != null ? row.associatedBusiness : '-' }}</div>
					</template>
				</el-table-column>
				<el-table-column prop="floor" label="关联楼栋" min-width="200" show-overflow-tooltip align="left">
					<template #default="{ row, $index }">
						<div>{{ row.floor != null ? row.floor : '-' }}</div>
					</template>
				</el-table-column>
				<el-table-column prop="earlyAlarmState" width="120" label="预警状态" show-overflow-tooltip align="center">
					<template #default="scope">
						<div v-if="scope.row.earlyAlarmState == '1'" class="contract-saved">未预警</div>
						<div v-else-if="scope.row.earlyAlarmState == '2'" class="contract-submit">预警中</div>
						<div v-else-if="scope.row.earlyAlarmState == '3'" class="contract-audit">已完成</div>
					</template>
				</el-table-column>
				<el-table-column prop="alarmState" width="120" label="报警状态" show-overflow-tooltip align="center">
					<template #default="scope">
						<div v-if="scope.row.alarmState == '1'" class="contract-saved">未报警</div>
						<div v-else-if="scope.row.alarmState == '2'" class="contract-submit">报警中</div>
						<div v-else-if="scope.row.alarmState == '3'" class="contract-audit">已完成</div>
					</template>
				</el-table-column>

				<!-- <el-table-column prop="yujing" width="260" label="预警内容" show-overflow-tooltip align="center">
					<template #default="scope">
						<div v-if="scope.row.associatedBusiness">{{ scope.row.associatedBusiness + '-' + scope.row.specificPointName + ',未开始' }}</div>
						<div v-else>-</div>
					</template>
				</el-table-column>
				<el-table-column prop="baojing" width="260" label="报警内容" show-overflow-tooltip align="center">
					<template #default="scope">
						<div v-if="scope.row.associatedBusiness">{{ scope.row.associatedBusiness + '-' + scope.row.specificPointName + ',未完成' }}</div>
						<div v-else>-</div>
					</template>
				</el-table-column> -->
					...
					<template #default="scope">
						<el-tooltip class="box-item" :offset="0" effect="light" placement="top">
							...
							<el-button
								...
								type="primary"
								class="option-btn"
								:disabled="plan.state == 1 || scope.row.associateLog"
						...
						<el-tooltip class="box-item" :offset="0" effect="light" placement="top">
							<template #content><span style="color: #3668f7 !important">关联业务</span></template>
							<el-button
								v-show="!scope.row.isEdit"
								icon="Connection"
								text
								...
								:disabled="plan.state == 1 || scope.row.associateLog"
								v-auth="'progress_dpmTaskPlan_add'"
								@click="relateWork(scope.row, scope.$index)"
							></el-button>
						</el-tooltip>
						<el-tooltip class="box-item" :offset="0" effect="light" placement="top">
							...
							<el-button
								...
								class="option-btn"
								type="primary"
								:disabled="scope.row.floor != null || plan.state == 1 || scope.row.associateLog"
								v-auth="'progress_dpmTaskPlan_add'"
						...
						<el-tooltip class="box-item" :offset="0" effect="light" placement="top">
							...
							<el-button
								...
								type="primary"
								:disabled="plan.state == 1 || scope.row.associateLog"
								v-auth="'progress_dpmTaskPlan_edit'"
						...
						<el-tooltip class="box-item" :offset="0" effect="light" placement="top">
							...
							<el-button
								...
								class="option-btn"
								type="primary"
								:disabled="plan.state == 1 || (scope.row.hasChild ? scope.row.associateChildLog : scope.row.associateLog)"
			...
			<RelateWorkDialog ref="relateWorkDialogRef" :reloadPlanInfo="reloadPlanInfo" :reloadNode="reloadNode" @loadingStart="loadingStart" />
		</div>
	</div>
</template>
...
import { fetchList, deleteTask, add, putObj, publish, move, versionEdit, checkName } from '/@/api/task/taskPlan';
...
const RelateWorkDialog = defineAsyncComponent(() => import('./relateWorkDialog.vue'));
...
const dataRules = ref({
	...
	planBeginDate: [{ required: true, message: '计划开始时间不能为空', trigger: ['blur', 'change'] }],
	planEndDate: [{ required: true, message: '计划完成时间不能为空', trigger: ['blur', 'change'] }],
});
...
watch(
	...
	(newValue, oldValue) => {
		if (newValue.projectId) {
			nextTick(() => {
				selectedId.value = String(newValue.projectId);
				tableData.value = [];
				store.ppData = [];
...
const cellBell = ({ row, column, rowIndex, columnIndex }) => {
	return (!row.planBeginDate || !row.planEndDate) && (columnIndex == 4 || columnIndex == 5) ? 'redTag' : '';
};
...
const relateWork = (row, index) => {
	// console.log(row, index);
	relateWorkDialogRef.value.openDialog(row);
	parentRow.value = row;
};
...
const reloadPlanInfo = (id, parentId) => {
	let versionId: string = plan.value.versionIdVal;
	return fetchList({ id, parentId, versionId }).then((res) => {
		if (res) {
			plan.value.state = res.data.state != null ? res.data.state : 0;
			plan.value.publishTime = res.data.publishTime != null ? res.data.publishTime : '';
			plan.value.taskCount = res.data.taskCount != null ? res.data.taskCount : 0;
...
const reloadNode = (row) => {
	let versionId: string = plan.value.versionIdVal;
	return fetchList({ id: row.projectId, parentId: row.parentId, versionId }).then((res) => {
		// debugger;
		if (res) {
			res.data.taskPlanInfoList.map((v) => {
				v.add = false;
...
const copyRow = async (row, index) => {
	...
	try {
		...
		if (row.parentId == '0') {
			...
			newRow.isTop = false;
			newRow.nodeStr = '';
			editTreeTable.value.store.states.lazyTreeNodeMap.value[row.parentId].push(newRow);
		}
...
const saveRow = (refs, row) => {
	...
	// }
	// if (isEmpty.length > 0) {
	if (!row.taskName || !row.planBeginDate || !row.planEndDate) {
		return false;
	} else {
		checkName(row).then((res) => {
			if (res.data) {
				useMessage().error('任务名称重复');
			} else {
				save(row);
			}
		});
